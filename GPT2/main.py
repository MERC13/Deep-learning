...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        B, T = idx.size()
        assert T <= self.config.block_size, f"Cannot foward sequence of length {T}..."
...
for step in range(max_steps):
    ...
    print(f"step {step} | loss: {loss_accum.item():.6f} | lr: {lr:.4e} | norm: {norm:.4f} | dt: {d1:.2f}ms | tok/sec: {tokens_per_sec:.2f}")
